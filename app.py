...
class Music(commands.Cog):
    ...
    async def repeat_command(self, ctx: commands.Context):
        ...
        if self.voice_client and (self.voice_client.is_playing() or self.voice_client.is_paused()):
            if not self.data:
                await ctx.reply("**إذا كـنـت تــقـرأ الــرســالـة يــرجــى التــواصــل مــع المطور**")
            else:
                if self.data.get('user') and self.data['user'] != ctx.author:
    ...
    async def play_command(self, ctx: commands.Context, *, message: Optional[str] = None):
        if ctx.author.voice is None:
    ...
    async def skip_command(self, ctx: commands.Context):
        ...
        if self.voice_client and (self.voice_client.is_playing() or self.voice_client.is_paused()):
            if not self.data:
                await ctx.reply("**إذا كـنـت تــقـرأ الــرســالـة يــرجــى التــواصــل مــع المطور**", delete_after=10)
            else:
                if self.data.get('user') and (self.data['user'] != ctx.author and self.data['user'] in ctx.author.voice.channel.members):
                    await ctx.reply(f"**:x: Only The Music Player ({self.data['user'].mention}) Can Use This Command**")
                else:
                    self.skip = True
                    self.isloop = False
    ...
    async def stop_command(self, ctx: commands.Context):
        ...
        if self.voice_client:
            if self.data and self.data.get('user') and (self.data['user'] != ctx.author and self.data['user'] in ctx.author.voice.channel.members):
                await ctx.reply(f"**:x: Only The Music Player ({self.data['user'].mention}) Can Use This Command**")
            else:
